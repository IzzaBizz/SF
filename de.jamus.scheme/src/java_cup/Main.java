...
public class Main {
	...
	protected static String arg_text = ""; // <------F
	...
	public static void main(String argv[]) throws internal_error,
		...
		File textf; // <------F
		textf = new File(arg_text); // <------F
		InputStream instr; // <------F
		instr = new FileInputStream(textf); // <------F
		input_file = new BufferedInputStream(instr); // <------F
		...
		parse_grammar_spec();
		System.err.println("Parsing specification is over ...");
	...
	protected static void parse_args(String argv[]) {
		...
		for (i = 0; i < len; i++) {
			...
			} else if (argv[i].equals("-grammar")) // <------F
			{
				++i;
				arg_text = argv[i]; // <------F
				System.out.println("Dateiname: " + arg_text); // <------F
				// <------F
			} // <------F
	...
	protected static void parse_grammar_spec() throws java.lang.Exception {
		...
		parser_obj = new parser(input_file);
		try {
			if (opt_do_debug)
				parser_obj.debug_parse();
			else {
				System.out.println("++ Parse is starting"); // <------F
				// parser_obj.parse();
				parser_obj.parse(); // <------F
				System.out.println("++ Parse is over"); // <------F
			}
		} catch (Exception e) {
			/*
